...
# Alternativt: mirt_object <- readRDS("~/shiny-server/valkompass2014/mirt_object.RDS")

scores <- fscores(mirt_obj, rotate="entropy") %>% as.data.frame %>% mutate(F1 = -F1)
candidates <- c(valkompass, scores) %>% as_tibble
